...
<div class="wrap">
 <div class="bdp-header-logo">
  <img src="<?php echo esc_url( BLITZ_DOCK_URL . 'assets/images/blitz-dock.png' ); ?>" alt="Blitz Dock" width="200" height="auto" />
</div>

  <div class="bdp-admin-container">
    <ul class="bdp-tabs" role="tablist">
      <?php foreach ( $tabs as $slug => $label ) :
        $icon_file = isset( $tab_icons[ $slug ] ) ? $tab_icons[ $slug ] : ''; ?>
        <li role="presentation">
             <button id="tab-<?php echo esc_attr( $slug ); ?>"
                  role="tab"
                  aria-controls="panel-<?php echo esc_attr( $slug ); ?>"
                  aria-selected="<?php echo $slug === 'dashboard' ? 'true' : 'false'; ?>">
            <img class="bdp-tab-icon" src="<?php echo esc_url( $menu_icons_url . $icon_file ); ?>" alt="" aria-hidden="true" />
            <span class="bdp-tab-label"><?php echo esc_html( $label ); ?></span>
          </button>
        </li>
      <?php endforeach; ?>
    </ul>
 <div class="bdp-panels">
      <div id="panel-dashboard" class="bdp-tab-panel active" role="tabpanel" aria-labelledby="tab-dashboard">
            <?php
          $msg_table  = $wpdb->prefix . 'bdp_messages';
    ...
    <!-- SSS – FAQ -->
               <div id="panel-help" class="bdp-tab-panel" role="tabpanel" aria-labelledby="tab-help">
      ...
      <div id="bdp-faq-items">
        <?php foreach ( $faq_items as $i => $faq ) : ?>
          ...
          <div class="bdp-faq-row" style="display:flex;flex-direction:column;gap:8px;margin-bottom:1rem;">
            <input type="text"
                   name="bdp_faq_items[<?php echo $i; ?>][question]"
                   value="<?php echo esc_attr( $faq['question'] ); ?>"
                   placeholder="<?php esc_attr_e( 'Soru Başlığı', 'blitz-dock' ); ?>" />
            <textarea name="bdp_faq_items[<?php echo $i; ?>][answer]" rows="3"
                      placeholder="<?php esc_attr_e( 'Cevap', 'blitz-dock' ); ?>"><?php echo esc_textarea( $faq['answer'] ); ?></textarea>
            <button type="button" class="button bdp-remove-faq">Sil</button>
          </div>
        <?php endforeach; ?>
      </div>
      <button type="button" class="button" id="bdp-add-faq"><?php esc_html_e( '+ Yeni SSS Ekle', 'blitz-dock' ); ?></button>
    </div>

    <!-- Sosyal Medya -->
   <div id="panel-social" class="bdp-tab-panel" role="tabpanel" aria-labelledby="tab-social">
    ...
    <!-- Dock Ayarları -->
   <div id="panel-settings" class="bdp-tab-panel" role="tabpanel" aria-labelledby="tab-settings">
    ...
    <!-- Online Mağazalar -->
    <div id="panel-ecomm" class="bdp-tab-panel" role="tabpanel" aria-labelledby="tab-ecomm">
   ...
   <!-- Location -->
    <div id="panel-location" class="bdp-tab-panel" role="tabpanel" aria-labelledby="tab-location">
      <h2><?php esc_html_e( 'Location Embed', 'blitz-dock' ); ?></h2>
        <div class="bdp-location-flex-wrap">
        <div class="bdp-location-form">
          <textarea name="bdp_location_embed" id="bdp_location_embed" rows="4"><?php echo esc_textarea( $location_embed ); ?></textarea>
          <p class="description"><?php esc_html_e( 'Paste your Google Maps iframe code here.', 'blitz-dock' ); ?></p>
        </div>
        <div class="bdp-location-preview" id="bdp-location-preview">
          <?php if ( $location_embed ) : ?>
            <div class="bdp-map-responsive">
              <?php echo wp_kses_post( html_entity_decode( $location_embed ) ); ?>
            </div>
          <?php else : ?>
            <p class="bdp-map-placeholder"><?php esc_html_e( 'Map preview will appear here after embedding your Google Maps iframe code.', 'blitz-dock' ); ?></p>
          <?php endif; ?>
        </div>
      </div>
      <?php submit_button(); ?>
...
$extra_panels = [
  ...
  'live_chat' => [ '\\BlitzDock\\AdminUI\\Live_Chat_Admin', 'render_page' ],
];
foreach ( $extra_panels as $slug => $cb ) {
  echo '<div id="panel-' . esc_attr( $slug ) . '" class="bdp-tab-panel" role="tabpanel" aria-labelledby="tab-' . esc_attr( $slug ) . '">';
  call_user_func( $cb );
  echo '</div>';
}
?>
